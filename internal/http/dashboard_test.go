...
func TestBuildDashboardViewModel(t *testing.T) {
	now := time.Now().UTC()
	vm := buildDashboardViewModel([]evidence.Item{{Status: "expiring", OwnerEmail: "a@b.com"}, {Status: "expired"}, {Status: "active"}}, []map[string]any{{"id": "1", "created_at": now}}, 10, "memory", true)
