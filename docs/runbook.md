...
```bash
# Docker
docker tag evidencevault:previous evidencevault:latest
docker compose up -d --force-recreate

# Kubernetes
kubectl rollout undo deployment/evidencevault
```

### Logs
```bash
# Structured JSON logs
docker logs -f evidencevault | jq .

# Filter by trace_id
docker logs evidencevault | grep "trace_id=\"abc123\""
```
